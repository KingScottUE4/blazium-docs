...
.. XML source: https://github.com/blazium-games/blazium/tree/4.3/doc/classes/JSON.xml.
...
\ **Note:** The JSON specification does not define integer or float types, but only a *number* type. If ``filesystem/import/json/always_parse_numbers_as_double`` is ``true``, converting a Variant to JSON text will convert all numerical values to :ref:`float<class_float>` types. Otherwise, it will convert to either :ref:`float<class_float>` or :ref:`int<class_int>`.
